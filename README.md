...
1. **侧栏「更多工具」折叠组** —— 把 技能与工具 / 消息平台 / 产物 / 定时任务 等导航行收进一行
   **「更多工具」**，点击原地展开收起，**默认折叠**。以后用官方贡献面 `sidebar.nav` 注册的插件导航行
   **会自动收进这一组**，不用改本插件。组内还有三行：**内置浏览器 / 文档预览 / 会议记录**（点它们都在会话区整块打开）。
2. **搜索会话框上移到侧栏最上** —— 加一道 1px 若隐若现的淡灰细框，去掉原生「空值时半透明」，
   占位文字颜色加深（都在「新建会话」之上）。
3. **「已置顶」默认折叠** —— 进应用时自动折一次；之后完全听你手动开关，不跟你抢。
4. **会话区排版** —— 行高 1.62、段距 0.95rem、轮间距 0.6rem（浅深双套自动）。可一键关。
5. **用户气泡配色** —— 整块界面里**唯一**的颜色改动：「你发的消息」那颗气泡淡蓝渐变
   （浅色 `#4a90d0`→`#8dc0e8`，深色 `#6ea8e8`→`#a9d2f2`）+ 同色系描边。可一键关。
...
| 想干什么 | 怎么做 |
|---|---|
| 展开 / 收起侧栏工具组 | 点侧栏「更多工具」；状态记上次（默认折叠） |
